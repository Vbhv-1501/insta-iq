...
async def check_and_increment_usage(db: AsyncSession, user: User) -> None:
    ...
    result = await db.execute(select(UsageRecord).where(UsageRecord.user_id == user.id, UsageRecord.date == today))
    record = result.scalar_one_or_none()
    if record and record.analyses_count >= limit:
        raise HTTPException(status_code=429, detail=f"Daily limit reached ({limit}/day on {user.plan} plan).")
    if record:
...
async def create_pending_report(db: AsyncSession, user_id: str, username: str) -> AnalysisReport:
    report = AnalysisReport(id=str(uuid.uuid4()), user_id=user_id, username=username, status="pending")
    db.add(report)
    await db.commit()
...
async def get_report(db: AsyncSession, report_id: str, user_id: str) -> Optional[AnalysisReport]:
    result = await db.execute(select(AnalysisReport).where(AnalysisReport.id == report_id, AnalysisReport.user_id == user_id))
    return result.scalar_one_or_none()


async def get_reports_paginated(db: AsyncSession, user_id: str, page: int = 1, per_page: int = 20) -> tuple:
    offset = (page - 1) * per_page
    count_result = await db.execute(select(func.count()).select_from(AnalysisReport).where(AnalysisReport.user_id == user_id))
    total = count_result.scalar_one()
    result = await db.execute(select(AnalysisReport).where(AnalysisReport.user_id == user_id).order_by(AnalysisReport.created_at.desc()).offset(offset).limit(per_page))
...
async def run_analysis(db: AsyncSession, report_id: str, username: str, user: User) -> None:
    ...
    try:
        ...
        if profile_data.get("is_private"):
            raise ValueError(f"@{username} is private. InstaIQ only analyzes public accounts.")
...
def _enrich_followers(raw: list, source: str) -> list:
    enriched = []
    for f in raw:
        bio = f.get("bio", "") or ""
        full_name = f.get("full_name", "") or ""
        combined = f"{bio} {full_name}".strip()
        lang = detect_language(combined)
        country, confidence = detect_country(bio, lang, full_name)
        bot_score = compute_bot_score(f)
        enriched.append({**f, "source_username": source, "detected_language": lang, "detected_country": country, "country_confidence": confidence, "bot_score": bot_score})
    return enriched


async def _bulk_upsert_followers(db: AsyncSession, followers: list) -> None:
    for f in followers:
        result = await db.execute(select(Follower).where(Follower.source_username == f["source_username"], Follower.follower_username == f["username"]))
        ...
        if existing:
            ...
            db.add(Follower(id=str(uuid.uuid4()), source_username=f["source_username"], follower_username=f["username"], follower_full_name=f.get("full_name"), follower_bio=f.get("bio"), is_verified=f.get("is_verified", False), is_private=f.get("is_private", False), followers_count=f.get("followers_count", 0), following_count=f.get("following_count", 0), post_count=f.get("post_count", 0), detected_language=f.get("detected_language"), detected_country=f.get("detected_country"), country_confidence=f.get("country_confidence", 0.0), bot_score=f.get("bot_score", 0.0)))
    await db.commit()


async def _upsert_profile(db: AsyncSession, data: dict) -> None:
    from datetime import timedelta
    result = await db.execute(select(InstagramProfile).where(InstagramProfile.username == data["username"]))
    ...
    if profile:
        ...
        allowed = {c.key for c in InstagramProfile.__table__.columns}
        db.add(InstagramProfile(id=str(uuid.uuid4()), cache_expires_at=expires, **{k: v for k, v in data.items() if k in allowed}))
    await db.commit()
...
def _estimate_engagement(profile_data: dict) -> float:
    ...
    if posts == 0:
        return 0.0
    return round(min(max(0.5, 8.0 - math.log10(followers) * 0.9), 10.0), 1)


async def _generate_ai_summary(username: str, profile: dict, analytics: dict) -> tuple:
    try:
        ...
        prompt = f'Analyze @{username}. Followers: {profile.get("followers_count",0):,}, Quality: {quality}/100, Bots: {bot_pct}%, Top country: {top_country}, Language: {top_lang}. Write 2-sentence summary and 4 insights. JSON only: {{"summary":"...","insights":["...","...","...","..."]}}'
        resp = await client.chat.completions.create(model=settings.OPENAI_MODEL, messages=[{"role": "user", "content": prompt}], response_format={"type": "json_object"}, max_tokens=400)
        data = json.loads(resp.choices[0].message.content)
        return data.get("summary", ""), data.get("insights", [])
    except Exception as e:
        log.warning("ai_fallback", error=str(e))
        country_dist = analytics.get("country_distribution") or []
        ...
        return (
            f"@{username} has a quality score of {quality}/100 with the largest audience in {top_country}. Based on {sample} sampled followers.",
            [f"Bot rate of {bot_pct}% is {'above' if bot_pct > 15 else 'below'} the 15% platform average.", f"Inactive rate of {analytics.get('inactive_pct',0)}% suggests audience churn.", "Compare with similar accounts to benchmark quality.", "Target content toward the primary language group for better engagement."]
        )
